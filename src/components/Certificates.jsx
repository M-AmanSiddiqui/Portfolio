...
function Achievements() {
  ...
  useEffect(() => {
    document.body.style.overflow = (selectedCertificate || showFullImage) ? "hidden" : "auto";
    return () => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a1a] text-white px-4 sm:px-6 md:px-10 py-10 flex flex-col items-center">
      ...
      {/* Grid View */}
      {!selectedCertificate && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mt-10 gap-6 w-full max-w-6xl">
          {achievements.map((cert, index) => (
            <motion.div
              key={index}
...
{selectedCertificate && (
  ...
  >
    ...
    >
      ...
      <h2 className="text-xl sm:text-2xl font-semibold text-[#915EFF]">{selectedCertificate.title}</h2>
      <p className="text-white text-sm sm:text-base">{selectedCertificate.provider} - {selectedCertificate.date}</p>
